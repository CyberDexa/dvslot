-- DVSlot Database Schema for Supabase
...
CREATE TABLE public.alerts (
  ...
  notification_types notification_type[] DEFAULT ARRAY['push'],
  last_notification_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- User Alert Subscriptions (for multiple test centers)
...
CREATE TABLE public.user_preferences (
  ...
  alert_frequency INTEGER DEFAULT 15, -- minutes
...
-- Users can only see and manage their own alert subscriptions
ALTER TABLE alert_subscriptions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own subscriptions" ON alert_subscriptions FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own subscriptions" ON alert_subscriptions FOR INSERT WITH CHECK (auth.uid() = user_id);
